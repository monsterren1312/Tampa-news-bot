...
Парсит RSS-ленты новостей Тампы/Флориды, переводит на русский через Anthropic API,
публикует в Telegram-канал (@TampaNews1) с картинками (если есть в источнике).
...
log = logging.getLogger("tampa-news-bot")
...
CHANNEL_SIGNATURE = "🌴 Tampa News"
CHANNEL_URL = os.environ.get("CHANNEL_URL", "https://t.me/TampaNews1")

# Источники — можно добавлять сколько угодно, каждый со своим именем.
# Если какой-то источник временно недоступен или сменил RSS-адрес,
# бот просто пропустит его в этом запуске и продолжит с остальными.
RSS_SOURCES = [
    {"name": "FOX13 Tampa", "url": "https://www.fox13news.com/feed"},
    {"name": "WFLA News Channel 8", "url": "https://www.wfla.com/feed"},
    {"name": "10 Tampa Bay (WTSP)", "url": "https://www.wtsp.com/feeds/syndication/rss/news/local"},
    {"name": "Bay News 9", "url": "https://www.baynews9.com/fl/tampa/feed"},  # проверить точный адрес при первом запуске
]
...
def rewrite_in_russian(title: str, summary: str, source_name: str) -> str | None:
    ...
    prompt = f"""Ты редактор Telegram-канала новостей города Тампа (Флорида) на русском языке.
